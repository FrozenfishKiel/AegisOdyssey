...
#include "AegisOdyssey/System/AOAssetManager.h"
...
void UGF_AddInputMapping::AddInputMappingForPlayer(UPlayer* Player, FPerContextData& ActiveData)
{
...
void UGF_AddInputMapping::RegisterInputMappingContextsForLocalPlayer(ULocalPlayer* LocalPlayer)
{
	...
	{
		UAOAssetManager& AOAssetManager = UAOAssetManager::Get();
		...
		if (UEnhancedInputLocalPlayerSubsystem* EISubsystem = ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(LocalPlayer))
		{
			if (UEnhancedInputUserSettings* Settings = EISubsystem->GetUserSettings())
			{
				...
				for (const FInputMappingContextAndPriority& Entry : InputMappings)
				{
					...
					if (UInputMappingContext* IMC = AOAssetManager.GetAsset(Entry.InputMapping))
					{
		...
		if (UEnhancedInputLocalPlayerSubsystem* EISubsystem = ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(LocalPlayer))
		{
			if (UEnhancedInputUserSettings* Settings = EISubsystem->GetUserSettings())
			{
				for (const FInputMappingContextAndPriority& Entry : InputMappings)
				{
					if (Entry.Priority == 0)
					{
						Settings->UnregisterInputMappingContext(Entry.InputMapping.Get());  //如果InputMapping路径已加载则直接获取
					}
				}
			}
		}
