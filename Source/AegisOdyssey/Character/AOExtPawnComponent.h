...
DECLARE_MULTICAST_DELEGATE(FOnASCWasAssign);
...
public:
	...
	void SetDataPawn(const UAOPawnData* InPawnData);

	//返回创建的对应类的PawnData
	template <class T>
	const T* GetPawnData() const { return Cast<T>(DefaultPawnData); }
public:
	void CallRegister_OnASCWasAssign(FOnASCWasAssign::FDelegate&& OnASC);
protected:
...
protected:
	UPROPERTY(Transient)
	TObjectPtr<UAOAbilitySystem> AbilitySystemComponent;
...
private:
	...
	void TryFinalizeAbilitySystemInitialization();
	void ScheduleAbilitySystemInitializationFinalize();
	void ClearAbilitySystemInitializationFinalizeTimer();
	void HandleAbilitySystemDataChanged();
	void RefreshPrimaryAttributesFromCurrentLevel();
	void HandleCharacterLevelChanged(int32 OldLevel, int32 NewLevel);
	void BindCharacterLevelChanged();
	void UnbindCharacterLevelChanged();

private:
	FOnASCWasAssign OnASCWasAssignDelegate;
	FAOAbilitySet_GrantedHandles PawnDataGrantedHandles;
	FTimerHandle AbilitySystemInitializationFinalizeTimerHandle;
	bool bAbilitySystemLevelBindingReady = false;
	bool bAbilitySystemInitializationFinalized = false;
};
