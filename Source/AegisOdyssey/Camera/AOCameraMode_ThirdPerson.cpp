...
#include "AOCameraMode_ThirdPerson.h"
#include "Curves/CurveVector.h"
#include "GameFramework/Character.h"
#include "GameFramework/CameraBlockingVolume.h"
#include "AegisOdyssey/Camera/AOCameraAssistInterface.h"
#include "AegisOdyssey/Camera/AOPenetrationAvoidanceFeeler.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(AOCameraMode_ThirdPerson)

namespace AOCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UAOCameraMode_ThirdPerson::UAOCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FAOPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FAOPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FAOPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FAOPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FAOPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FAOPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FAOPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));

}

void UAOCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UAOCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UAOCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IAOCameraAssistInterface* TargetControllerAssist = Cast<IAOCameraAssistInterface>(TargetController);

	IAOCameraAssistInterface* TargetActorAssist = Cast<IAOCameraAssistInterface>(TargetActor);
	...
	IAOCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IAOCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IAOCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IAOCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UAOCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc,
	FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
	...
	//TODO IAOCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FAOPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(AOCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UAOCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UAOCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
