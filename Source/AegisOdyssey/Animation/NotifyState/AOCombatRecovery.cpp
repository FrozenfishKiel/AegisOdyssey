...
#include "AOCombatRecovery.h"

#include "AbilitySystemBlueprintLibrary.h"
#include "AegisOdyssey/Character/AOCharacter.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(AOCombatRecovery)

UAOCombatRecovery::UAOCombatRecovery()
{
	CombatRecoveryTag = FGameplayTag();
}

void UAOCombatRecovery::NotifyBegin(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation, float TotalDuration,
                                    const FAnimNotifyEventReference& EventReference)
{
	...
	// 添加连招窗口标签
	if (CombatRecoveryTag.IsValid())
	{
		ASC->AddLooseGameplayTag(CombatRecoveryTag);
		//UE_LOG(LogTemp, Log, TEXT("AOCombatWindow: Added CombatWindowTag to %s"), *AOCharacter->GetName());
	}
}

void UAOCombatRecovery::NotifyEnd(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
	const FAnimNotifyEventReference& EventReference)
{
	...
	if (CombatRecoveryTag.IsValid())
	{
		ASC->RemoveLooseGameplayTag(CombatRecoveryTag);
