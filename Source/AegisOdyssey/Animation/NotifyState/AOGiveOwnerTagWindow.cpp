...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AOGiveOwnerTagWindow)

UAOGiveOwnerTagWindow::UAOGiveOwnerTagWindow()
{
}

void UAOGiveOwnerTagWindow::NotifyBegin(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
	float TotalDuration, const FAnimNotifyEventReference& EventReference)
{
	...
	if (GiveTag.IsValid())
	{
		ASC->AddLooseGameplayTag(GiveTag);
		//UE_LOG(LogTemp, Log, TEXT("AOCombatWindow: Added CombatWindowTag to %s"), *AOCharacter->GetName());
	}}

void UAOGiveOwnerTagWindow::NotifyEnd(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
	const FAnimNotifyEventReference& EventReference)
{
	...
	if (GiveTag.IsValid())
	{
		ASC->RemoveLooseGameplayTag(GiveTag);
